...
type Props = {
  ...
  /** Сохранён для совместимости с SymbolSearchModal; в этом UI не используется. */
            ...
            {loadError ? (
              ...
              filtered.map((c) => (
                <li key={c.id}>
                  ...
                  >
                    <img src={c.iconUrl} alt="" className="asset-picker-item-icon" />
                    <div className="asset-picker-item-text">
                      <p className="asset-picker-item-symbol">{c.symbol}</p>
                      <p className="asset-picker-item-name">{c.name}</p>
                    </div>
                  </button>
                </li>
              ))
